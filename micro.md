...
Cluster, the referencing endpoint or waypoint and attached resources

Clustering, forming local cells of an endpoint or waypoint, relaypoints and resources to enhance the capability of the cluster.  Relaypoints offer direct routes of network connection, and waypoints send their network activity through the nearest relaypoint, the waypoints authenticate each session through their endpoint and maintain authentication throughout their session for their network activity generated by the cluster’s use
...
Attaching:
Attach... starting a phrase ending with “Confirm”, attaches a resource if the resource will increase the locally available memory, storage, network connectivity, computational ability, modes of interfacing, modes of interactivity or power, to the cluster and the user is authorized to do so by the resource’s owning or managing entity, confirmed as a rule 

Unattaching:
Unattach... starting a phrase ending with “Confirm”, unattaches a resource from the locally available resources to the cluster which the user is authorized to do so with by the resource’s owning or managing entity, confirmed as a rule
