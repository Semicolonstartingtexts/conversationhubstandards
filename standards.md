...
Relaypoint, any appropriated resource or declared relaypoint or resource which has a direct route by way of network connectivity to the endpoint and associated backend
...
Attach... When given starting a system command, attaches a resource if the resource will increase the locally available memory, storage, network connectivity, computational ability, modes of interfacing, modes of interactivity or power, and the user is authorized to do so by the resource’s owning or managing entity, letting the command giving user notice if the resource will act as a relaypoint, and how much more is available to be done with a previously done server interaction could be done now
