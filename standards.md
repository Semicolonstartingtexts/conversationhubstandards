...
Preferrings, the conversation which governs the server’s, endpoint, waypoint, node, relay or the user’s own self’s interactions with the user’s self, set and stacked only by the user and only on the user, or on a mapping, if the mapping’s provisions permit, preferrings, can’t be unstacked by other users than a user’s own self 
