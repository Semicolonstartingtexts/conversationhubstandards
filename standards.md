...
Relay, an object imbued with the functionality of a server, the imbuing is tracked by the endpoint the relay was imbued on, keeping the owner and roles of the endpoint of where the object was imbued and the roles of the server which the object is relaying, but assigning an owner of the relay to the user which does the imbuing, the relay has the same users as the server at the same time and the same allowed limited number of users
