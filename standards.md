...
Clear the representation of/lose the system speech… clears the representation of the described system speech in the phrase from users which the user is authorized to and the library shelves, after the user confirms their intention to do so with a prompted noticing of the categories of system speech which will be cleared and amounts following the prompt noticed by the user asking the user if there sure they would like to clear the representation of… and the system speech description they described abandoning the system command’s interaction if the user declines at either prompt, if starting a system command
