...
Suspend the… and an endpoint or waypoint, when starting a system command, suspends the definition of the user’s owned endpoint or waypoint’s server or servers referenced or all servers on an endpoint or waypoint when the endpoint or waypoint is said to be suspended after performing the hangup reserved system command for and on all users on the servers of the endpoint or waypoint or referenced server declining contacting and connections and system commands while a server is being suspended and the users active calls ending until the time the server is resumed or endpoint or waypoint’s servers are resumed is which they hold an owner role of and what’s stacked on the endpoint or waypoint or server
