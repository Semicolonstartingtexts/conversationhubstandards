...
Imbuing, stacking on, or tracking conversation or a signifier or multiple signifiers with the representation of an intention or intentions involving an object, imbued objects are limited to inheriting and do inherit only from the definition of or rules for general or specific use of the nearest endpoint, if the imbuing server was stacked on an endpoint, or nearest waypoint in the inheritance chain they were imbued on through the imbuing unless the imbuing is removed, though the same object may be imbued by different endpoints or waypoints simultaneously
